...
const activity_at_first_inj = (patient_inj_time_list, rp_settings) => {
  const ta = decay(
    rp_settings.rp_activity,
    rp_settings.rp_half_life,
    diff_time(patient_inj_time_list[0], rp_settings.mesure_time)
  );
  const ra =
    (ta * (rp_settings.rp_vol - rp_settings.wasted_vol)) / rp_settings.rp_vol;
  return ra;
};

const usable_activity = (
  total_rp_activity,
  total_rp_vol,
  unextractable_rp_vol
) => {
  const a =
    (total_rp_activity * (total_rp_vol - unextractable_rp_vol)) / total_rp_vol;
  return a;
};

const generate_patient_inj_time_list = (
  patient_list,
  patient_scan_time_list,
  rp_settings
) => {
  ...
  patient_list.forEach((x, i) => {
    if (x.injected) {
      ...
      patient_inj_time_list[i] = new Date(
        patient_inj_time_list[i - 1]
      ).setMinutes(
        new Date(patient_inj_time_list[i - 1]).getMinutes() +
          patient_scan_time_list[i - 1]
      );
    }
  });
  ...
  while (sorting_condition) {
    ...
    for (var i = 0; i < patient_list.length - 1; i++) {
      if (patient_list[i].injected === false) {
        ...
        let before = calcul_final_expected_activity(
          patient_list,
          rp_settings
        ...
        let after = calcul_final_expected_activity(
          patient_list,
          rp_settings
        ).usable_remaining_activity;
...
const activity_now = (patient_list, rp_settings) => {
  ...
  injected_patients_list = injected_patients_list.map((x) => ({
    ...x,
    inj_time: x.inj_time ? new Date(x.inj_time) : null,
  }));
  rp_settings = {
    ...rp_settings,
    mesure_time: new Date(rp_settings.mesure_time),
    first_inj_time: new Date(rp_settings.first_inj_time),
  };

  if (k === 0) {
    now_dict.total_vol_now = rp_settings.rp_vol - rp_settings.wasted_vol;
    now_dict.usable_vol_now =
      now_dict.total_vol_now - rp_settings.unextractable_vol;
    now_dict.total_activity_now_before_prime = decay(
      rp_settings.rp_activity,
      rp_settings.rp_half_life,
      diff_time(new Date(), rp_settings.mesure_time)
    );
    now_dict.total_activity_now =
      (now_dict.total_activity_now_before_prime * now_dict.total_vol_now) /
      rp_settings.rp_vol;
    now_dict.usable_activity_now = usable_activity(
      now_dict.total_activity_now,
      now_dict.total_vol_now,
      rp_settings.unextractable_vol
    );
  } else {
    ...
    for (var i = 0; i < k; i++) {
      if (i === 0) {
        inj_time_activity_list[i] = activity_at_first_inj(
          patient_inj_time_list,
          rp_settings
        );
        remaining_activity_list[i] =
          inj_time_activity_list[i] - patient_dose_list[i];
        patient_inj_vol_list[i] =
          (patient_dose_list[i] *
            (rp_settings.rp_vol - rp_settings.wasted_vol)) /
          inj_time_activity_list[i];
        remaining_vol_list[i] =
          rp_settings.rp_vol - rp_settings.wasted_vol - patient_inj_vol_list[i];
      } else {
        inj_time_activity_list[i] = decay(
          remaining_activity_list[i - 1],
          rp_settings.rp_half_life,
          diff_time(patient_inj_time_list[i], patient_inj_time_list[i - 1])
        );
        remaining_activity_list[i] =
          inj_time_activity_list[i] - patient_dose_list[i];
        patient_inj_vol_list[i] =
          (patient_dose_list[i] * remaining_vol_list[i - 1]) /
          inj_time_activity_list[i];
        remaining_vol_list[i] =
          remaining_vol_list[i - 1] - patient_inj_vol_list[i];
      }
    }

    now_dict.total_vol_now = remaining_vol_list[k - 1];
    now_dict.usable_vol_now =
      remaining_vol_list[k - 1] - rp_settings.unextractable_vol;
    now_dict.total_activity_now = decay(
      remaining_activity_list[k - 1],
      rp_settings.rp_half_life,
      diff_time(new Date(), patient_inj_time_list[k - 1])
    );
    now_dict.usable_activity_now = usable_activity(
      now_dict.total_activity_now,
      remaining_vol_list[k - 1],
      rp_settings.unextractable_vol
    );
  }
...
const sorting_after_every_injection = (patient_list) => {
  patient_list.sort((a, b) => {
    return (
      (a.inj_time === null) - (b.inj_time === null) ||
      +(a.inj_time > b.inj_time) ||
      -(a.inj_time < b.inj_time)
    );
  });
};
...
const calcul_final_expected_activity = (patient_list, rp_settings) => {
  ...
  let patient_inj_time_list = generate_patient_inj_time_list(
    patient_list,
    patient_scan_time_list,
    rp_settings
  );
  ...
  patient_dose_list.forEach((x, i) => {
    if (i === 0) {
      ...
      remaining_activity_list[i] =
        inj_time_activity_list[i] - patient_dose_list[i];
      patient_inj_vol_list[i] =
        (patient_dose_list[i] * (rp_settings.rp_vol - rp_settings.wasted_vol)) /
        inj_time_activity_list[i];
      remaining_vol_list[i] =
        rp_settings.rp_vol - rp_settings.wasted_vol - patient_inj_vol_list[i];
    } else {
  ...
  const usable_remaining_activity = usable_activity(
    remaining_activity_list.slice(-1)[0],
    remaining_vol_list.slice(-1)[0],
    rp_settings.unextractable_vol
  );
  ...
  const expected = {
    ...
    usable_remaining_vol: (
      remaining_vol_list.slice(-1)[0] - rp_settings.unextractable_vol
    ).toFixed(2),
